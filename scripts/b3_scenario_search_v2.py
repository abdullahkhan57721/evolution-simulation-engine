"""Run narrowed B3 environment-dependent selection experiments.

Temporary analysis instrumentation only. This is not a public package surface or
...
class IndividualStateRecorder:
    """Record analysis-only per-organism focal speed and energy by committed step."""
...
def _patches(*, radius: int) -> PatchyResourcePlacement:
    return PatchyResourcePlacement(
        patches=(
            ResourcePatch(center_x=2, center_y=5, radius=radius),
            ResourcePatch(center_x=9, center_y=5, radius=radius),
        )
    )
...
def _environment_specs() -> dict[str, tuple[int, object]]:
    return {
        ...
        "patch_r1_d16": (16, _patches(radius=1)),
        "patch_r2_d16": (16, _patches(radius=2)),
        "uniform_d32": (32, UniformResourcePlacement()),
        "patch_r1_d32": (32, _patches(radius=1)),
        "patch_r2_d32": (32, _patches(radius=2)),
    }
...
def _run_one(*, label: str, seed: int, deposits: int, placement: object) -> dict[str, Any]:
    ...
    examples: dict[int, dict[str, Any] | None] = {LOW_SPEED: None, HIGH_SPEED: None}
            ...
            if target_x is not None:
                ...
                if examples[speed] is None:
                    ...
                    examples[speed] = {
                        ...
                        "speed": speed,
                        "start": [None if new_x is None else new_x - dx, None if new_y is None else new_y - dy],
                        "end": [new_x, new_y],
                        "target": [target_x, target_y],
                        "movement_energy_cost": cost,
                        "resource_consumed_same_step": consumption_by_step_id.get((completed_step, organism_id), 0),
                        "energy_before_step": recorder.energy_by_step.get(completed_step - 1, {}).get(organism_id),
                        "energy_after_step": recorder.energy_by_step.get(completed_step, {}).get(organism_id),
                    }
    ...
    extinction_step = next((item["step"] for item in population_history if item["population"] == 0), None)
    final_nonzero_genetic = next(
        (item for item in reversed(genetic_history) if item["population"] > 0),
        genetic_history[0],
    )
    ...
    return {
        ...
        "last_nonzero_step": final_nonzero_genetic["step"],
        "last_nonzero_high_allele_frequency": final_nonzero_genetic["high_allele_frequency"],
        ...
        "mechanism": {
            ...
            "founder_reproductive_success": {str(k): _distribution(v) for k, v in founder_reproduction.items()},
            "all_reproductive_success": {str(k): _distribution(v) for k, v in all_reproduction.items()},
        },
        "movement_examples": {str(k): v for k, v in examples.items()},
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("outputs/b3-analysis/b3-scenario-search-v2.json"))
