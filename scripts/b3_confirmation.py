...
def run_confirmation() -> tuple[dict[str, Any], tuple[str, ...]]:
    """Run all predeclared confirmation, sensitivity, and counterbalance cases."""
    ...
    checks = {
        ...
        "compact_beats_uniform_at_least_6_of_8": patch_wins >= 6,
        ...
        "uniform_founder_rrs_favors_low_at_least_6_of_8": uniform_low_rrs_wins >= 6,
        "compact_founder_rrs_favors_high_at_least_6_of_8": compact_high_rrs_wins >= 6,
        "compact_resources_stay_inside_compact_support": all(
            pair.treatment.resource_geography.compact_support_fraction == 1.0
            for pair in pairs
        ),
        "uniform_resources_not_confined_to_compact_support": all(
            pair.control.resource_geography.compact_support_fraction is not None
            and pair.control.resource_geography.compact_support_fraction < 1.0
            for pair in pairs
        ),
        "broad_patch_weakens_compact_advantage_in_aggregate": broad_mean < treatment_mean,
        "swapped_founder_assignment_preserves_patch_advantage": (
            counterbalance_patch_wins == len(B3_COUNTERBALANCE_SEEDS)
        ),
    }

    representative_seed, median_effect = _select_representative_seed(pairs)
    representative_pair = next(pair for pair in pairs if pair.seed == representative_seed)
    ...
    payload: dict[str, Any] = {
        ...
        "aggregate": {
            "uniform_step_30_mean_high_speed_allele_frequency": control_mean,
            "compact_step_30_mean_high_speed_allele_frequency": treatment_mean,
            "broad_patch_step_30_mean_high_speed_allele_frequency": broad_mean,
            "mean_compact_minus_uniform_effect": mean_effect,
            "compact_greater_than_uniform_seed_count": patch_wins,
            "uniform_low_speed_founder_rrs_win_count": uniform_low_rrs_wins,
            "compact_high_speed_founder_rrs_win_count": compact_high_rrs_wins,
            "counterbalance_compact_win_count": counterbalance_patch_wins,
        },
