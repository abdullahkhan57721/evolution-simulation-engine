"""Renderer-neutral world-presentation adapters for exact Workbench run artifacts.

The module converts already-recorded spatial/trait evidence into the shared
``WorldPresentationFrame`` contract and keeps exact Study/run/treatment context
beside it. It never calculates scientific outcomes or reconstructs missing evidence.
"""
...
class WorkbenchWorldPresentation:
    """Keep exact Workbench context beside one shared world frame."""
...
def build_reference_workbench_world_presentation(
    ...
    *,
    ...
    """Build a world frame only from spatial evidence actually recorded by WB4."""
...
def build_b3_workbench_world_presentation(
    ...
    *,
    ...
    """Build one matched B3 frame while preserving arm/replicate identity."""
