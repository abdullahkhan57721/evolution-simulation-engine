"""UI-only helpers for WU2 Simulation authoring over concrete Workbench contracts."""
...
def normalize_reference_draft(intent: ReferenceEcologyIntent) -> ReferenceEcologyIntent:
    ...
    This is transient UI-state hygiene. Saved scientific normalization remains owned
    by ``fork_reference_study_revision`` / ``create_reference_study_revision``.
    """
    if not isinstance(intent, ReferenceEcologyIntent):
        raise TypeError("intent must be a ReferenceEcologyIntent.")
...
def save_controlled_child(
    ...
    *,
    ...
    """Create an immutable controlled child revision from a ready UI draft."""
