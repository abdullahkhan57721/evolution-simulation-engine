"""Frontend-neutral helpers for the concrete Workbench experiment patterns."""

from __future__ import annotations

import attrs

from evo_engine.workbench.b3_curated import B3StudyRevision, compile_b3_curated
...
def parse_integer_sequence(value: str, *, name: str) -> tuple[int, ...]:
    """Parse a comma-separated frontend field into integer values."""
...
def max_speed_run_rows(
    ...
    """Build frontend rows only from the authoritative Workbench E3 expansion."""
    return tuple(
        MaxSpeedRunRow(
...
def environment_run_rows(
    ...
    """Build frontend rows only from the authoritative Workbench E4 expansion."""
    return tuple(
        EnvironmentRunRow(
            seed=treatment.seed,
            role=treatment.role,
...
def e4_counterbalance_label() -> str:
    """Return a scientific frontend label while retaining authoritative identity."""
