...
- `evo_engine.genetics` remains upstream of development, energetics, engine orchestration, processes, reproduction, resolvers, spatial behavior, and world state.
- Domain packages (`development`, `energetics`, `genetics`, `reproduction`, `spatial`, and `world`) must not depend on concrete process or resolver implementations.
