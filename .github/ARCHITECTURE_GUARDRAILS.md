...
- `evo_engine.genetics` remains upstream of behavior, development, energetics, engine orchestration, growth, processes, reproduction, resolvers, spatial behavior, and world state.
- Domain packages (`behavior`, `development`, `energetics`, `genetics`, `growth`, `reproduction`, `spatial`, and `world`) must not depend on concrete process or resolver implementations.
