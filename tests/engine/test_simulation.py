...
def test_simulation_exposes_arbitrary_shared_service() -> None:
    """Test named domain configuration remains accessible through context."""
    service = object()
    ...
    assert simulation.scoring_rule is service
...
def test_missing_context_attribute_raises_attribute_error() -> None:
    """Test missing services use normal Python attribute semantics."""
    simulation = Simulation(initial_world_state=CounterState())

    with pytest.raises(AttributeError, match="missing_service"):
        _ = simulation.missing_service
