...
def test_world_state_advance_time_increases_time_step_by_one(
    current_time_step
):
    # Arrange
    world_state = WorldState(time_step=current_time_step, organisms=[])
...
@pytest.mark.parametrize(
    "organisms",
    [
        [Organism(id=0), Organism(id=1),],
        [Organism(id=3), Organism(id=23),],
        [Organism(id=0), Organism(id=34), Organism(id=32),],
    ]
)    
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [AgingEvent(organism_id=1, time_step=4), AgingEvent(organism_id=17, time_step=4)]
        ), 
    ]
)
def test_state_updater_applies_aging_events(
    organisms,
    events
):
    ...
    updated_world_state = state_updater.apply(
