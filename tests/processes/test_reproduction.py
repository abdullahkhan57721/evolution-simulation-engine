...
class ThreeParticipantSelection:
    """Propose the first three eligible organisms as one reproductive group."""
    ...
    def propose_reproductive_groups(
        ...
        *,
        ...
        """Return one three-participant group when enough organisms are eligible."""
        ...
        return [
            ReproductiveGroup(
                participant_ids=tuple(
                    reference_model.reference(participant, state=world)
                    for participant in eligible_participants[:3]
                )
            )
        ]
...
def test_reproduction_materializes_three_participant_group() -> None:
    """Test shared reproduction orchestration imposes no one/two-member ceiling."""
    ...
    expected_ids = tuple(participant.id for participant in participants)
    assert proposals[0].participant_ids == expected_ids
    assert proposals[0].initial_energy == 6
    assert event.participant_ids == expected_ids
    assert event.parent_ids == expected_ids
    assert event.offspring_genome == participants[0].genome


def test_one_participant_proposal_records_energy_contribution() -> None:
    """Test clonal proposal captures the participant's committed investment."""
    architecture = make_integer_architecture("offspring_energy")
...
def test_proposal_omits_group_that_cannot_afford_investment() -> None:
    """Test affordability before proposals enter resolution."""
    state = make_state()
    add_organism(state, energy=4)
    process = Reproduction(
        eligibility=AlwaysEligible(),
        reproductive_group_selection=SingleParent(),
        inheritance_model=ClonalInheritance(),
...
def test_materialize_clonal_event_produces_genome_phenotype_and_location() -> None:
    ...
    architecture = make_integer_architecture("adult_body_mass", "offspring_energy")
    state = make_state(genetic_architecture=architecture)
    parent = add_organism(
        state,
        trait_values={"adult_body_mass": 8, "offspring_energy": 5},
        energy=20,
        x=3,
        y=4,
    )
...
def test_apply_event_is_mechanical_and_does_not_advance_rng() -> None:
    ...
    architecture = make_integer_architecture("offspring_energy")
    state = make_state(genetic_architecture=architecture, seed=10)
    parent = add_organism(
        state,
        trait_values={"offspring_energy": 5},
        energy=20,
...
def test_materialize_rechecks_recorded_participant_affordability() -> None:
    """Test stale resolved proposals cannot overdraw participant energy."""
    state = make_state()
...
def test_apply_rechecks_participant_affordability_atomically() -> None:
    """Test failed application does not partially charge participants."""
    architecture = make_integer_architecture("offspring_energy")
    state = make_state(genetic_architecture=architecture)
    first = add_organism(
        state,
...
def test_two_parent_materialization_uses_sexual_inheritance() -> None:
    """Test two resolved genetic parents contribute one haploid gamete each."""
    architecture = make_integer_architecture("offspring_energy")
    state = make_state(genetic_architecture=architecture, seed=4)
    first = add_organism(
        state,
    ...
    assert set(event.parent_ids) == {first.id, second.id}
...
def test_default_reproduction_materializes_current_mass_from_adult_target() -> None:
    ...
    add_organism(
        state,
        trait_values={"adult_body_mass": 7, "offspring_energy": 5},
        energy=20,
    )
    process = Reproduction(
        eligibility=AlwaysEligible(),
...
def test_reproduction_declares_only_configured_policy_trait_dependencies() -> None:
    ...
    assert default_process.required_traits == frozenset(
        {ADULT_BODY_MASS, OFFSPRING_ENERGY}
    )
...
def test_reproduction_materializes_development_after_genetic_expression() -> None:
    ...
    architecture = make_integer_architecture(ADULT_BODY_MASS, "offspring_energy")
    state = make_state(genetic_architecture=architecture, seed=1)
    add_organism(
        state,
        trait_values={ADULT_BODY_MASS: 20, "offspring_energy": 5},
        energy=20,
    )
    process = Reproduction(
        eligibility=AlwaysEligible(),
