"""Regression tests for retained V2 adaptive movement controls."""
...
def test_retained_gaussian_control_progressively_reveals_and_hides() -> None:
...
def test_retained_hidden_gaussian_value_cannot_affect_uniform_run() -> None:
    app = AppTest.from_file(str(_HARNESS_PATH)).run(timeout=30)
    app = _enter_custom_configuration(app)
...
def test_retained_gaussian_run_commits_selected_typed_variant() -> None:
    app = AppTest.from_file(str(_HARNESS_PATH)).run(timeout=30)
    app = _enter_custom_configuration(app)
