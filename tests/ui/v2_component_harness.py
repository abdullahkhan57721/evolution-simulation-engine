"""Test-only harness for retained V2 configuration/workspace components.

WU1 replaces the production application entry shell. This harness exists only so
the retained V2 configuration and world-workspace components remain behavior-tested
for later WU integration; it is not a second application entry point.
"""
...
def main() -> None:
    """Exercise the retained V2 component composition under Streamlit AppTest."""
    st.set_page_config(
        page_title="Retained V2 Component Harness",
        page_icon="🧬",
        layout="wide",
    )
    _initialize_mode()
