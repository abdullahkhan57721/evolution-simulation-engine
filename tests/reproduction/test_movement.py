...
def test_reproductive_eligibility_condition_requires_exact_boolean() -> None:
    ...
    condition = ReproductiveEligibilityMovementCondition(
        eligibility=_InvalidEligibility(),
    )
...
@pytest.mark.parametrize(
    ...
    [
        (_Compatibility(1), _FixedPreference(0)),
        (_Compatibility(True), _FixedPreference(True)),
    ],
)
def test_preferred_mate_target_validates_pair_policy_return_types(
    compatibility,
    preference,
) -> None:
    """Test target selection preserves mating policy return contracts."""
    state = make_state()
    focal = add_organism(state)
    add_organism(
        state,
